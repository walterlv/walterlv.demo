...
namespace MSTest.Extensions.Contracts
{
    ...
    public class ContractTestContext<T1, T2, T3, T4, T5>
    {
        /// <summary>
        /// Create a new <see cref="ContractTestContext{T1, T2, T3, T4, T5}"/> instance with contract description and testing action.
        /// </summary>
        ...
        public ContractTestContext([NotNull] string contract, [NotNull] Action<T1, T2, T3, T4, T5> testCase)
        {
            _contract = contract ?? throw new ArgumentNullException(nameof(contract));
            if (testCase == null) throw new ArgumentNullException(nameof(testCase));
#if NET45
#pragma warning disable CS1998 // Async function without await expression
            _testCase = async (t1, t2, t3, t4, t5) => testCase(t1, t2, t3, t4, t5);
#pragma warning restore CS1998 // Async function without await expression
#else
            _testCase = (t1, t2, t3, t4, t5) =>
            {
        ...
        /// <summary>
        /// Create a new <see cref="ContractTestContext{T1, T2, T3, T4, T5}"/> instance with contract description and async testing action.
        /// </summary>
        /// <param name="contract">The contract description string for this test case.</param>
        /// <param name="testCase">The actual async action to execute the test case.</param>
        public ContractTestContext([NotNull] string contract, [NotNull] Func<T1, T2, T3, T4, T5, Task> testCase)
        {
            _contract = contract ?? throw new ArgumentNullException(nameof(contract));
        ...
        /// </remarks>
        [NotNull, PublicAPI]
        public ContractTestContext<T1, T2, T3, T4, T5> WithArguments([CanBeNullWhenTIsSingle] params (T1, T2, T3, T4, T5)[] ts)
        {
            if (ts == null)
            {
#if GENERATED_CODE
            ...
            foreach (var (t1, t2, t3, t4, t5) in ts)
            {
                // If any argument is not formatted, post the argument value at the end of the contract string.
                var contract = string.Format(_contract, ForT(t1), ForT(t2), ForT(t3), ForT(t4), ForT(t5));
                if (!allFormatted)
                {
                    contract = contract + $"({ForT(t1)}, {ForT(t2)}, {ForT(t3)}, {ForT(t4)}, {ForT(t5)})";
                }

                // Add an argument test case to the test case list.
                ContractTest.Method.Current.Add(new ContractTestCase(contract, () => _testCase(t1, t2, t3, t4, t5)));
            }
        ...
        /// <returns>The instance itself.</returns>
        [NotNull, PublicAPI]
        public ContractTestContext<T1, T2, T3, T4, T5> WithArguments(T1 t1, T2 t2, T3 t3, T4 t4, T5 t5)
        {
            ContractTest.Method.Current.Add(new ContractTestCase(
                string.Format(_contract, t1, t2, t3, t4, t5), () => _testCase(t1, t2, t3, t4, t5)));
            return this;
        }
#endif
        ...
        /// </summary>
        [NotNull] private readonly Func<T1, T2, T3, T4, T5, Task> _testCase;
    }
}
